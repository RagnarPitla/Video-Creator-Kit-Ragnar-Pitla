...
if (typeof document !== 'undefined') {
	...
	style.textContent = FACES.map(
		...
		// If the woff files are not in public/fonts the @font-face rules simply
		// fail to load and the browser falls through to whatever it can find, so
		// name the closest system faces explicitly rather than landing on Times.
		.concat(
			'\nbody{font-family:"Segoe UI","Segoe UI Variable","Helvetica Neue",Arial,sans-serif;}',
		);
