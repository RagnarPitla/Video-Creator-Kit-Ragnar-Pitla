...
export const miaFilmSchema = z.object({
  ...
  /** Puts a Dynamics 365 lockup on the opening frame. Off in both shipped
   * cuts: review note 1 asked for the wording on that frame to go, so putting
   * a different line of wording in its place is not what was asked. Kept as a
   * switch because the option came up and is one prop away. */
  openingLogo: z.boolean(),
});
 ...
 * the D365 Mia title animation, the Success by Design slide, the screen
...
export const SLOTS: Slot[] = [
  ...
  {from: 2559, duration: 240, label: 'S11 Steady state', node: <S11Steady {...FILM_COPY.s11} />},
  {
    // Review note 11: pulled 60 frames earlier so the pull-back is already
    // running when "That is the vision" is spoken at film frame 2862.
    from: 2799,
    duration: 330,
  ...
  return (
    <AbsoluteFill style={{backgroundColor: '#EEF3F8'}}>
      ...
      <FilmGrade look={graded}>
        ...
        {openingLogo ? (
          <Sequence from={0} durationInFrames={282} name="D365 lockup">
            <BrandLockup />
          </Sequence>
        ) : null}
